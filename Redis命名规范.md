在电商秒杀系统中，缓存设计至关重要，而**缓存 key 的命名规范**直接影响缓存的可读性、维护性和有效性。一个好的命名规范能避免 key 冲突、便于问题排查，同时提升团队协作效率。以下是针对电商秒杀系统的缓存 key 命名规范设计方案：


### 一、核心设计原则
1. **唯一性**：避免不同业务的 key 冲突（例如用户信息和商品信息的 key 不能重复）。  
2. **可读性**：通过 key 能直观判断存储的内容和业务含义。  
3. **可维护性**：便于后续扩展和修改（例如增加新业务模块时无需大规模调整 key 规则）。  
4. **性能适配**：避免 key 过长（Redis 对 key 长度不限制，但过长会消耗内存和带宽），建议单个 key 长度控制在 100 字符以内。  
...
| 业务场景         | key 命名格式                     | 示例                          | 存储内容                  | 过期策略                |
|------------------|----------------------------------|-------------------------------|---------------------------|-------------------------|
| 商品基本信息     | `goods:info:{goodsId}`           | `goods:info:2001`             | 商品名称、图片、原价等    | 1 小时（更新时主动失效）|
